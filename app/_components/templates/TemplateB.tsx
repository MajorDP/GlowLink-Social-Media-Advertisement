...
function TemplateB({ data, isPublic }: ITemplate) {
  return (
    <div
      ...
      } bg-black via-violet-950 to-slate-950 shadow-xl shadow-teal-800 flex flex-col`}
    >
      {!isPublic && (
        <Link
        ...
        <div className="mt-6 flex flex-col justify-center gap-3 w-1/2">
            ...
            <div className="mt-6 flex flex-col gap-3">
              {data.additionalLinks.map((link, index) => (
                <Link
                  key={index}
                  href={link.link}
                  className="w-1/2 mx-auto bg-white text-black rounded-lg px-4 py-2 text-center hover:opacity-80 transition cursor-pointer"
                >
                  {link.name}
                </Link>
              ))}
          ...
          {data.donationsLink && (
            <Link
              ...
              className="w-1/2 mx-auto flex flex-row items-center justify-center px-4 rounded-lg py-2 bg-gradient-to-r from-pink-500 hover:from-pink-400 to-red-500 hover:opacity-90 text-white cursor-pointer"
            >
